...
class EDSR_v7:
    def __init__(self, args):
        ...
        self.apply_quantize = args.apply_quantize
    ...
    def _build_upsample(self, x):
        if self.upsample_type == 'transpose':
            x = ops.transpose_upsample(x, self.scale, self.channel_in, self.data_format, 5, self.num_reduced_filters, self.apply_quantize, True) #compact version
            ...
            x = ops.bilinear_upsample(x, self.scale, self.data_format, self.num_reduced_filters, self.apply_quantize, True) #False for training
            x = layers.Conv2D(self.channel_in,
                                        (3,3),
                                        padding='same',
                                        data_format=self.data_format)(x)
    ...
    def build(self):
        ...
        for _ in range(self.num_blocks):
            outputs = ops.residual_block_(x=outputs,
