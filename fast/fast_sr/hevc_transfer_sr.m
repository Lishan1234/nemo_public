...
img_area = (img_width / sr_ratio) * (img_height / sr_ratio);
...
% ------------------------------------------------------------------------
time_info.runtime_1 = zeros(1, N_frames);
time_info.runtime_2 = zeros(1, N_frames);
time_info.runtime_3 = zeros(1, N_frames);
time_info.runtime_4 = zeros(1, N_frames);
time_info.runtime_5 = zeros(1, N_frames);
time_info.runtime_6 = zeros(1, N_frames);
...
for f_idx = 2:N_frames
    ...
    time_info.runtime_1(f_idx) = time_info.runtime_1(f_idx) + toc;
    transfer_area = 0;
    for pu_idx = 1:length(hevc_info.other_info.PU{f_idx})
        ...
        switch PU_now(pu_idx).intra
            case 0
                ...
                time_info.runtime_2(f_idx) = time_info.runtime_2(f_idx) + toc;
                if mean_ssd < params.transfer_thresh
                    tic;
                    transfer_area = transfer_area + w * h;
                    %if mean_ssd ~= 0
                    %    disp(mean_ssd);
                    %end
                    ...
                    time_info.runtime_3(f_idx) = time_info.runtime_3(f_idx) + toc;
                    ...
                    time_info.runtime_4(f_idx) = time_info.runtime_4(f_idx) + toc;
            ...
            case 1
                ...
                time_info.runtime_5(f_idx) = time_info.runtime_5(f_idx) + toc;
            otherwise
                error('Intra indicator can only be 0 or 1 in PU!');
        end
    end
    time_info.runtime_transfer(f_idx) = toc;
    percent_transfer(f_idx) = (transfer_area / img_area) * 100;
    % --------------------------------------------------------------------
    ...
    if params.deblock == 1
        ...
        time_info.runtime_6(f_idx) = time_info.runtime_6(f_idx) + toc;
    end
