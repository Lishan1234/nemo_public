...
def validate_data_constraints(schema, src_op, debug=False):
    ...
    """
    max_input_length = schema.numInputs[-1]
    max_output_length = schema.numOutputs[-1]
    unused_input_data = []
    unused_output_data = []
    ...
    if max_input_length and len(src_op.input) > max_input_length:
        if debug:
            prYellow("Expected {} input(s) but found {}. One of the following inputs: {} from {} op "
                     "will not be used".
                     format(max_input_length, len(src_op.input), map(str, src_op.input[:]), src_op.op_type))
        else:
            extra = ['unused'] * (len(src_op.input) - max_input_length)
            unused_input_data = zip(src_op.input[max_input_length:], extra)
    ...
    if max_output_length and len(src_op.output) > max_output_length:
        if debug:
            ...
            unused_output_data = zip(src_op.output[max_output_length:], extra)
...
def setup_dry_run_messages(model, model_version):
    for i, src_op in enumerate(model.graph.node):
        ...
        if len(op_supported[0]) == 2:
            try:
                supported_version = map(int, get_op_schema(src_op.op_type, 0).keys())
            except IndexError:
...
def print_attribute_info():
    ...
    for key, value in attributes_supported.items():
        prYellow(row_format.format(str(key), map(str, value)))
...
def print_version_info():
    ...
    for op_name, schema_dict in OP_SCHEMA_REGISTRY.items():
        version_supported[op_name] = schema_dict.get_schemas().keys()
