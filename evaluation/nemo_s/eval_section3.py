...
if __name__ == '__main__':
    ...
    log_file = os.path.join(log_dir, 'eval_section3_latency_{}.txt'.format(id_to_name(args.device_id)))
    with open(log_file, 'w') as f:
        for content in args.content:
            ...
            bilinear_avg_throughput = 1000 / np.round(np.average(libvpx_latency(bilinear_latency_log_dir)), 2)
            ...
            for num_layers, num_filters in zip(args.baseline_num_blocks, args.baseline_num_filters):
                ...
                dnn_throughput = 1000 / np.average(libvpx_latency(os.path.join(dnn_log_dir, args.device_id)))
                dnn_avg_throughput.append(dnn_throughput)

            f.write('{}\t{}\t{}\t{}\t{}\n'.format(content, bilinear_avg_quality, '\t'.join(str(x) for x in dnn_avg_quality),
                bilinear_avg_throughput, '\t'.join(str(x) for x in dnn_avg_throughput)))

    #2. power log
    log_dir = os.path.join(args.dataset_rootdir, 'evaluation')
    os.makedirs(log_dir, exist_ok=True)
    log_file = os.path.join(log_dir, 'eval_section3_power_{}.txt'.format(id_to_name(args.device_id)))
    with open(log_file, 'w') as f:
        for content in args.content:
            lr_video_dir = os.path.join(args.dataset_rootdir, content, 'video')
            lr_video_file = os.path.abspath(glob.glob(os.path.join(lr_video_dir, '{}p*'.format(args.lr_resolution)))[0])
            ...
            #bilienar
            bilinear_log_dir = os.path.join('/ssd2/nemo-mobicom-backup', content, 'log', lr_video_name, id_to_name(args.device_id), 'monsoon')
            ...
            bilinear_total_playback_time = playback_time(np.average(current), id_to_name(args.device_id)) / (fps / bilinear_fps)
            ...
            for num_layers, num_filters in zip(args.baseline_num_blocks, args.baseline_num_filters):
                nemo_s = NEMO_S(num_layers, num_filters, scale, args.upsample_type)
                dnn_log_dir = os.path.join('/ssd2/nemo-mobicom-backup', content, 'log', lr_video_name, nemo_s.name, id_to_name(args.device_id), 'monsoon')
                ...
                dnn_avg_energy.append(dnn_total_energy / total_frame / 1000)
                dnn_fps = total_frame / time
                dnn_total_playback_time.append(playback_time(np.average(current), id_to_name(args.device_id)) / (fps / dnn_fps))
    ...
    log_dir = os.path.join(args.dataset_rootdir, 'evaluation')
    os.makedirs(log_dir, exist_ok=True)
    log_file = os.path.join(log_dir, 'eval_section3_temperature_{}.txt'.format(id_to_name(args.device_id)))
    print(log_file)
    with open(log_file, 'w') as f:
        for content in args.content:
            lr_video_dir = os.path.join(args.dataset_rootdir, content, 'video')
            ...
            #bilienar
            bilinear_log_dir = os.path.join('/ssd2/nemo-mobicom-backup', content, 'log', lr_video_name, id_to_name(args.device_id), 'flir')
            ...
            for num_layers, num_filters in zip(args.baseline_num_blocks, args.baseline_num_filters):
                nemo_s = NEMO_S(num_layers, num_filters, scale, args.upsample_type)
                dnn_log_dir = os.path.join('/ssd2/nemo-mobicom-backup', content, 'log', lr_video_name, nemo_s.name, id_to_name(args.device_id), 'flir')
            ...
            interval = 10
            for i in range(min_len // interval):
                f.write('{:.2f}\t{:.2f}'.format(bilinear_time[i * interval] / 1000 / 60, bilinear_temperature[i * interval]))
                for time, temperature in zip(dnn_time, dnn_temperature):
                    f.write('\t{:.2f}\t{:.2f}'.format(time[i * interval] / 1000 / 60, temperature[i * interval]))
                f.write('\n')
