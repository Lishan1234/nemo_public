...
class Tester:
    ...
    def distribution(self, lr_image_dir):
        self.quantizer.plot(self.encoder, lr_image_dir, self.qnt_image_dir)

    def entropy(self, lr_image_dir):
        self.measure_entropy(lr_image_dir)

    def quality(self, lr_image_dir, hr_image_dir, save_image=False):
        #quantization
...
if __name__ == '__main__':
    ...
    parser.add_argument('--min_percentile', type=float, nargs='+', required=True)
    parser.add_argument('--max_percentile', type=float, nargs='+', required=True)
    ...
    result_path = os.path.join(result_dir, 'quantization_ed_s.txt')
    os.makedirs(result_dir, exist_ok=True)

    with open(result_path, 'w') as f:
        f.write('#Block\t#Filter\tPercentile(Min)\tPercentile(Max)\tValue(Min)\tValue(Max)\tQuality(SR)\tQuality(SR-qnt)\tQuality(Bilinear)\tEntropy(RGB)\tEntropy(Feature)\n')
        for dec_num_blocks in args.dec_num_blocks:
            for dec_num_filters in args.dec_num_filters:
                ...
                for idx, percentile in enumerate(list(zip(args.min_percentile, args.max_percentile))):
                    result = []
                    min_percentile = percentile[0]
                    max_percentile = percentile[1]
                    linear_quantizer = LinearQuantizer(min_percentile, max_percentile)

                    checkpoint_dir = os.path.join(args.dataset_dir, 'checkpoint', ffmpeg_option.summary(args.lr_video_name), edsr_ed_s.name)
                    log_dir = os.path.join(args.dataset_dir, 'log', ffmpeg_option.summary(args.lr_video_name), edsr_ed_s.name, linear_quantizer.name)
                    image_dir = os.path.join(args.dataset_dir, 'image', ffmpeg_option.summary(args.lr_video_name), edsr_ed_s.name, linear_quantizer.name)
                    os.makedirs(checkpoint_dir, exist_ok=True)
                    os.makedirs(log_dir, exist_ok=True)

                    tester = Tester(edsr_ed_s, linear_quantizer, checkpoint_dir, log_dir, image_dir)
                    tester.quality(lr_image_dir, hr_image_dir, args.save_image)
                    tester.entropy(lr_image_dir)
                    if idx == 0:
                        tester.distribution(lr_image_dir)

                    quality_log_path = os.path.join(log_dir, 'quality.txt')
                    with open(quality_log_path, 'r') as f_q:
                        result = f_q.readline().strip().split('\t')
                        sr_quality = result[1]
                        sr_qnt_quality = result[2]
                        bilinear_quality = result[3]

                    entropy_log_path = os.path.join(log_dir, 'entropy.txt')
                    with open(entropy_log_path, 'r') as f_q:
                        result = f_q.readline().strip().split('\t')
                        rgb_entropy = result[1]
                        feature_entropy = result[2]

                    result.append(dec_num_blocks)
                    result.append(dec_num_filters)
                    result.append(min_percentile)
                    result.append(max_percentile)
                    result.append(linear_quantizer.min_value)
                    result.append(linear_quantizer.max_value)
                    result.append(sr_quality)
                    result.append(sr_qnt_quality)
                    result.append(bilinear_quality)
                    result.append(rgb_entropy)
                    result.append(feature_entropy)
