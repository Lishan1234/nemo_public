...
if __name__ == '__main__':
    ...
    #convert model
    checkpoint_dir = os.path.join(args.log_dir)
    assert(os.path.exists(checkpoint_dir))
    model = tf.keras.applications.VGG16(input_shape=IMG_SHAPE, include_top=True, weights='imagenet')
    dlc_profile = snpe_convert_model(model, TENSOR_SHAPE, checkpoint_dir)

    #run benchmark
    dlc_file = os.path.join(args.log_dir, dlc_profile['dlc_name'])
    json_file = snpe_benchmark_random_config(args.device_id, args.runtime, model.name, dlc_file, args.log_dir)
