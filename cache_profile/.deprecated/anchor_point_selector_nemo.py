...
class APS_NEMO():
    ...
    def _debug_select_anchor_point(self, cache_profile, ap_cache_profiles):
        max_estimated_quality = None
        max_avg_estimated_quality = None
        idx = None
        ...
        for i, ap_cache_profile in enumerate(ap_cache_profiles):
            estimated_quality = self._estimate_quality(cache_profile, ap_cache_profile)
            avg_estimated_quality = np.average(estimated_quality)
            video_quality.append(avg_estimated_quality)
            if idx is None or avg_estimated_quality > max_avg_estimated_quality:
                max_avg_estimated_quality = avg_estimated_quality
                max_estimated_quality = estimated_quality
                idx = i

        return idx, max_estimated_quality, video_quality
    ...
    def run(self, chunk_idx):
        ...
        log_file = os.path.join(log_dir, 'estimated_video_quality.txt')
        print(log_file)
        video_quality_per_iteration = []
        index_per_iteration = []
        count = 0
        while len(ap_cache_profiles) > 0:
            idx, estimated_quality, video_quality = self._debug_select_anchor_point(cache_profile, ap_cache_profiles)
            video_quality_per_iteration.append(video_quality)
            index_per_iteration.append(idx)
            ap_cache_profile = ap_cache_profiles[idx]
        ...
        with open(log_file, 'w') as f:
            f.write('{}\n'.format('\t'.join([str(x) for x in index_per_iteration])))
            for results in zip(*video_quality_per_iteration):
                f.write('{}\n'.format('\t'.join([str('{:.2f}'.format(x)) for x in results])))
