...
if os.path.isdir(os.path.join(os.path.abspath(os.path.dirname(__file__)), '../lib/benchmarks/')):
    sys.path.append(os.path.join(os.path.abspath(os.path.dirname(__file__)), '../lib/benchmarks/snpebm'))
...
from snpebm_writer import Writer
import common_utils
...
def _find_shell_binary_on_target(device, logger):
    ...
    if device.adb.check_file_exists(sh_path) == False:
        sh_cmd = 'which sh'
        sh_path = ''
...
def snpe_bench(program_name,args_list, device_msm_os_dict=None):
    try:
        ...
        common_utils.adb.logger = logger
        common_utils.common.logger = logger
        common_utils.device.logger = logger
        common_utils.env.logger = logger
        ...
        config = ConfigFactory.make_config(args_parser.config_file_path, args_parser.output_basedir_override, args_parser.device_id_override, args_parser.host_name,
                                           ...
                                           args_parser.userbuffer_mode, args_parser.perfprofile, args_parser.profilinglevel)
        if config is None:
            return
        logger.info(config)
        ...
        for device_id in config.devices:
            ...
            for bm in benchmarks:
                matches = [value for key,value in RUNTIMES.items() if bm.runtime_flavor_measure.startswith(key)]
                if matches :
                    ...
                    logger.error("The specified runtime with  %s is not a supported runtime, benchmarks will not be running with this runtime"%bm.runtime_flavor_measure)
            ...
            if len(benchmarks_ran) == 0:
                ...
                snpe_version = benchmarks_ran[0][1].get_snpe_version(config, logger)
                basewriter = Writer(snpe_version, benchmarks_ran, config, device_info, args_parser.sleep, logger)

                if args_parser.generate_json:
                    basewriter.writejson(os.path.join(results_dir, "benchmark_stats_{0}.json".format(config.name)), logger)
                basewriter.writecsv(os.path.join(results_dir, "benchmark_stats_{0}.csv".format(config.name)), logger)
