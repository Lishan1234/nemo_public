...
//  static const int MAX_FRAMES = 3000;
...
//      }
    ...
    *fb = out_buffer->vpx_fb;
    ...
    pthread_mutex_unlock(&mutex);
      ...
      LOGE("get buffer: id = %d", out_buffer->id);
  ...
  int release(int id) {
    ...
    LOGE("release buffer: id = %d, ref_count = %d, reduced = %d", id, buffer->ref_count, buffer->ref_count-1);
...
//    LOGE("release frame buffer %d", (*(int*)fb->priv) );
...
//    mobinas_cfg * setup_mobinas_cfg = setup(content_dir, input_video);
  ...
  if (err) {
    ...
    LOGE("finish frame buffer callback setup");
...
DECODER_FUNC(jint, vpxGetFrame, jlong jContext, jobject jOutputBuffer) {
  ...
  if (outputMode == kOutputModeYuv) {
    ...
    LOGE("vpxGetFrame: id = %d, ref_count = %d, plane values: %u %u %u",
            id, jfb->ref_count,(*(jfb->planes[0])), (*(jfb->planes[1])), (*(jfb->planes[2])));
  }
  return 0;
}
...
DECODER_FUNC(jint, vpxRenderFrame, jlong jContext, jobject jSurface,
  ...
  LOGE("vpxRenderFrame: id = %d, ref_count = %d, plane values: %u %u %u",
       id, srcBuffer->ref_count,(*(srcBuffer->planes[0])), (*(srcBuffer->planes[1])), (*(srcBuffer->planes[2])));
