...
#include "com_example_snpe_libvpx_MainActivity.h"
...
//    mobinas_cfg->mode = DECODE_SR_CACHE;
    ...
    int result = decode_test(mobinas_cfg);
    __android_log_print(ANDROID_LOG_ERROR, "JNITAG", "hi decode success? %d",result);
         ...
         jint jint2)
{
    ...
    __android_log_print(ANDROID_LOG_ERROR, "JNITAG", "files: %s, %s, %s",hr_video_file,lr_video_file,sr_hq_video_file);
