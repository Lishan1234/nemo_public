...
{
  __android_log_print(ANDROID_LOG_ERROR, "JNITAG", "hi: %s",inputFile.c_str());

   std::ifstream in(inputFile, std::ifstream::binary);
   if (!in.is_open() || !in.good())
   {
    if(!in.is_open()){
            __android_log_print(ANDROID_LOG_ERROR, "JNITAG", "file not open");

    }else{
            __android_log_print(ANDROID_LOG_ERROR, "JNITAG", "file state not good");
            __android_log_print(ANDROID_LOG_ERROR, "JNITAG", "%d", in.eof());
            __android_log_print(ANDROID_LOG_ERROR, "JNITAG", "%d", in.fail());
            __android_log_print(ANDROID_LOG_ERROR, "JNITAG", "%d", in.bad());
            __android_log_print(ANDROID_LOG_ERROR, "JNITAG", "%d", in.rdstate());

    }
    //   std::cerr << "Failed to open input file: " << inputFile << "\n";
    //   __android_log_print(ANDROID_LOG_ERROR, "JNITAG", "can't open input file");
   }
   ...
   if (length % sizeof(T) != 0) {
      ...
      __android_log_print(ANDROID_LOG_ERROR, "JNITAG", "size of input file not divisible by sizeof(dtype)");
      return;
      // std::exit(EXIT_FAILURE);
   }

  __android_log_print(ANDROID_LOG_ERROR, "JNITAG", "Length is %zd",length);
  __android_log_print(ANDROID_LOG_ERROR, "JNITAG", "Float size is %zd",sizeof(T));
   ...
   if (!in.read(reinterpret_cast<char*>(&loadVector[0]), length))
   {
      ...
      __android_log_print(ANDROID_LOG_ERROR, "JNITAG", "Failed to read the contents of: %s", inputFile.c_str());
      __android_log_print(ANDROID_LOG_ERROR, "JNITAG", "Hyunho legnth: %d", length);
