...
int fake_main(const char * name, const char * b, const char * d, const char * i, const char * o,int minutes)
{
    enum {UNKNOWN, USERBUFFER_FLOAT, USERBUFFER_TF8, ITENSOR};
    enum {CPUBUFFER, GLBUFFER};
    ...
    else if(bufferType == ITENSOR)
    {
        ...
        while((diff = difftime(now,begin)) < seconds){
            ...
            __android_log_print(ANDROID_LOG_ERROR, "TIME", "bye");
...
//            }
            ...
            __android_log_print(ANDROID_LOG_ERROR, "TIME", "hi");
        }
