...
class ModelInfo(object):
    ...
    def dump_info(self, input_file_name, output_file_name):
        ...
        try:
            ...
            raise Exception("Error Querying for HTA Records in model:", self.model_filename, e)
    ...
    def dump_aix_info(self, aix_records, csv_content):
        ...
        print_value("\nHTA Info:", csv_content)
        headers = ["HTA Record Name", "nnc_version", "hta_driver_version", "record_version", "record_size", "Subnets"]
        col_sizes = [1 + len(header) for header in headers]
    ...
    def is_aix_enabled(self):
        ...
        for fileinfo in archive_filelist:
            if "hta" in fileinfo.filename or "aix" in fileinfo.filename:
                return True
        return False
