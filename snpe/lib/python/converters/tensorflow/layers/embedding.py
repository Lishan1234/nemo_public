...
from converters.tensorflow.common import LayerDescriptor, LayerBuilder, LayerResolver, ConverterError
                ...
                if all(tensor.op.type == 'Placeholder' for tensor in inputs_sanitized):
                    ...
                    if ids_candidate_shape == params_candidate_shape:
                        ...
                        if len(ids_candidate) == 0 or len(params_candidate) == 0:
                            raise ConverterError(get_message('ERROR_TF_EMBEDDING_CANNOT_RESOLVE_PARAMS_AND_IDS'))
                    else:
...
class EmbeddingLayerBuilder(LayerBuilder):
    def build_layer(self, converter_context, descriptor, input_descriptors, output_descriptors):
        ...
        if len(input_descriptors) != 2:
            raise ConverterError(get_message('ERROR_TF_EMBEDDING_REQUIRES_2_INPUTS'))
        ...
        return converter_context.model.add_embedding_layer(name=descriptor.layer_name,
                                                           ...
                                                           partition_strategy=snpe.modeltools.EMBEDDING_PARTITION_STRATEGY_MOD)
