...
class SnapDnnCppDroidBenchmark:
    ...
    def __create_script(self):
        ...
        if self.name in UB_MODES:
            run_cmd += UB_MODES[self.name]
        if self.debug:
            run_cmd += " --debug"
        if self.perfprofile:
        ...
        cmds.append(run_cmd)
        if not os.path.exists(self.host_rootpath):
            os.makedirs(self.host_rootpath)
    ...
    def post_commands(self):
        ...
        return [BenchmarkCommand("chmod 777 {0}".format(device_output_dir)),
                BenchmarkCommand("pull {0} {1}".format(os.path.join(self._exe_dir,SNPE_BENCH_SCRIPT),
                                                       self.host_result_dir), shell=False),
