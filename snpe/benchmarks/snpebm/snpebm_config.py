...
class DnnModel:
    def __init__(self, config, host_artifacts, logger):
...
class Config:
    def __init__(self, cfg_from_json, outputbasedir, devicelist, hostname, deviceostype, logger, userbuffer_mode, perfprofile, profilinglevel):
        ...
        if deviceostype == CONFIG_DEVICEOSTYPES_LE:
            ...
            self._stl_library = STL_GNUSTL_SHARED
            self._artifacts_config = load_json(os.path.join("snpebm", SNPE_BENCH_LE_OE_GCC64_ARTIFACTS_JSON), self._logger)
        elif deviceostype == CONFIG_DEVICEOSTYPES_LE64_OE_GCC64:
            self._architectures = [ARCH_AARCH64]
            self._platform_os = PLATFORM_OS_LINUX
            self._compiler = COMPILER_GCC64
            self._stl_library = STL_GNUSTL_SHARED
            self._artifacts_config = load_json(os.path.join("snpebm", SNPE_BENCH_LE64_OE_GCC64_ARTIFACTS_JSON), self._logger)
        elif deviceostype:
        ...
        self._dnnmodel = DnnModel(self._cfg_from_json, self.host_artifacts, logger)
    ...
    def return_valid_run_flavors(self):
        ...
        if self.userbuffer_mode != '':
            if self.userbuffer_mode in UB_MODES:
            ...
            if self.buffertypes is None:
                for runtime in flavors:
                    flavors[runtime].extend(RUNTIME_UB_MODES[runtime])
            else:
                if not all(map(lambda x: x in UB_MODES, self.buffertypes)):
                    raise ConfigError('Wrong userbuffer mode specified in config file')
                for runtime in flavors:
        ...
        if RUNTIME_GPU in runtimes and \
            ...
            for runtime in flavors:
                flavors[runtime] = filter(lambda x: x in RUNTIME_UB_MODES[runtime] + [''], flavors[runtime])
        return [(i, j) for i in runtimes for j in flavors[i]]
