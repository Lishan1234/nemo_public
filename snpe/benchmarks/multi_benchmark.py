...
args.snpe_project_root = os.path.abspath('../')
...
def setup_prerequisites():
    ...
    os.system('adb push $SNPE_ROOT/bin/$SNPE_TARGET_ARCH/snpe-throughput-net-run /data/local/tmp/snpeexample/$SNPE_TARGET_ARCH/bin')
...
def execute_network(runtime):
    ...
    if runtime == 'CPU':
        ...
        dlc_name = '{}.dlc'.format(model_name)
        runtime_opt = '--use_gpu'
        output_dir = '{}p_gpu'.format(args.lr*args.scale)
    elif runtime == 'GPU_FP16':
        dlc_name = '{}.dlc'.format(model_name)
        runtime_opt = '--use_gpu_fp16'
        output_dir = '{}p_gpu_fp16'.format(args.lr*args.scale)
    ...
    cmds = ['export SNPE_TARGET_ARCH=aarch64-android-clang6.0',
            ...
            'snpe-throughput-net-run --duration 20 --perf_profile high_performance --container {} {} -storage_dir ./{}'.format(os.path.join(dst_root_dir, args.train_data, 'models', model_name, 'dlc', dlc_name), runtime_opt, output_dir),
            'exit']
...
def calculate_psnr(runtime):
    ...
    if runtime == 'CPU':
        ...
        output_dir = os.path.abspath(os.path.join(args.snpe_project_root, 'custom', args.train_data, 'data', '{}p_gpu_fp16'.format(args.lr*args.scale)))
