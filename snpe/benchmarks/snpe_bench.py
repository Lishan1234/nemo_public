...
def snpe_bench(program_name,args_list, device_msm_os_dict=None):
    try:
        ...
        for device_id in config.devices:
            ...
            # Tune required device params to eliminate run to run variations
            if args_parser.set_system_params:
                device_prelim_settings(device, logger)
                if(device_msm_os_dict != None):
                    soc = device_msm_os_dict[device_id][1]
                    fix_device_params(device, soc, logger)
                else:
                    soc = get_device_soc(device, logger)
                    fix_device_params(device, soc, logger)
                test_device_access(device, logger)
            ...
            if len(benchmarks_ran) == 0:
                ...
                #hyunho: saved path is modified
                os.system('rm -rf {}'.format(results_dir))
                results_dir = os.path.abspath(os.path.join(results_dir, os.pardir))
                os.system('rm -rf {}'.format(os.path.join(results_dir, 'latest_results')))
