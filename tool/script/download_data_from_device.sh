    ...
    do
        ...
        do
            ...
            do
                ...
                CUDA_VISIBLE_DEVICES=${gpu_index} python ${NEMO_ROOT}/tool/download_data_from_device.py --data_dir ${NEMO_ROOT}/data --device_data_dir ${device_data_dir} --content ${content}${index} --video_name ${resolution}p_${bitrate}kbps_s0_d300.webm --num_blocks ${num_blocks} --num_filters ${num_filters} --algorithm=${algorithm} --device_id=${device_id}
            done
        done
    done
done
